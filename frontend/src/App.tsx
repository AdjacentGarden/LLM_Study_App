...
import {
  ...
  MemoryReturn,
  RememberQuestion,
} from "./components/LearningReturn";
...
function App() {
  ...
  function revisitDoubt(doubt: Doubt) {
    ...
    setAskedQuestion("");
    setError("");
    setQaQuestion(doubt.question);
    setView("qa");
    setNotice("已带回原来的问题，你可以补充后发送");
  }
  ...
  useEffect(() => {
    ...
    if (qaBook)
      void api
        .structure(qaBook.book_id)
        .then((value) => {
          if (!cancelled) setQaStructure(value);
        })
        .catch(() => {
          /* Generic suggestions remain available. */
        });
    return () => {
      cancelled = true;
    };
  ...
  async function loadCatalog(
    ...
    try {
      ...
      const savedId = preferredBook
        ? safeGet(`${SESSION_KEY}:${preferredBook}`)
        : safeGet(SESSION_KEY);
      const [catalog, restored] = await Promise.all([
        api.books(),
        savedId
          ? api.resume(savedId).catch((error) => {
      ...
      const matching =
        restored?.profile.book_id === active?.book_id ? restored : null;
      setBooks(catalog);
      setBook(active);
      ...
      if (matching)
        safeSet(
          `${SESSION_KEY}:${matching.profile.book_id}`,
          matching.session_id,
        );
    } catch (value) {
      if (generation === boot.current) setError((value as Error).message);
    } finally {
      if (generation === boot.current && showLoading) setInitializing(false);
    }
  }
  ...
  useEffect(() => {
    ...
    void api
      .userProfile()
      .then((value) => {
        if (!cancelled) setUserProfile(value);
      })
      .catch(() => {
        /* Profile page offers retry; keep the last saved avatar during network failures. */
      });
    return () => {
      cancelled = true;
    };
    ...
    const timer = window.setInterval(() => {
      void api
        ...
        .then((catalog) => {
          ...
          setBook((current) =>
            current?.book_id === id
              ? (catalog.find((item) => item.book_id === id) ?? current)
              : current,
          );
        })
        .catch(() => {
          /* Keep the existing catalog during a transient outage. */
        });
    }, 15000);
  ...
  async function submitInterview() {
    ...
    try {
      if (turn.phase === "profile_confirmation") {
        ...
        if (!wantsRevision) {
          setView("home");
          setNotice("学习方向已保存，看看建议先学的章节");
        }
        return;
      }
      if (turn.phase === "adaptive_diagnosis" && !turn.item) {
        acceptInterview(await api.next(session.session_id));
        return;
      }
      const labels = turn.options
      ...
      if (
        ...
        acceptInterview(
          await api.answerProfile(session.session_id, answer, selected),
        );
        return;
      }
      if (turn.item) {
        ...
        startedAt.current = Date.now();
        setNotice(
          `第 ${next.profile.diagnostic_observations.length} 次回答已记录，学习方向持续更新`,
        );
      }
    } catch (value) {
      setError((value as Error).message);
  ...
  async function reviewCard(rating: "again" | "hard" | "good" | "easy") {
    ...
    try {
      ...
      setActivity(result);
      setCourseNeedsRefresh((current) => current || result.course_stale);
      setSession({ ...session, profile: result.profile });
      setCardIndex((current) =>
        Math.min(course.flashcards.length - 1, current + 1),
      );
      setCardFlipped(false);
      startedAt.current = Date.now();
    } catch (value) {
  ...
  async function submitPractice() {
    ...
    const item = course.practice_items[practiceIndex];
    if (!item) return;
    setBusy(true);
    setError("");
    try {
    ...
    <StudioProvider>
      ...
      >
        <section className="phone" aria-label="云径手机端模拟界面">
          ...
          <header
            className={`app-header ${view === "interview" || view === "course" || view === "account" ? "context-header" : "root-header"}`}
          >
            {(view === "interview" ||
              view === "course" ||
              view === "account") && (
              <button
                ...
                aria-label="返回"
                onClick={() => setView(view === "account" ? "profile" : "home")}
              >
                <Icon name="back" />
              </button>
            )}
          ...
          <div
            ...
            key={view}
            inert={
              busy && !["qa", "interview", "course", "account"].includes(view)
            }
            className={`app-content page-transition ${view === "qa" ? "qa-shell" : view === "social" ? "social-shell" : ""} ${view === "interview" || view === "course" ? "focused-content" : ""}`}
          >
            {!online && (
                ...
                uploadEntry={
                  <BookUpload
                    ...
                    onAdded={async () => {
                      ...
                      setBooks(next);
                      if (!book && next[0]) await loadCatalog(next[0].book_id, false);
                    }}
                  />
                }
              >
                {book && <StudioEntry anchor={{ book_id: book.book_id }} />}
                {session && (
                  <MemoryReturn
            ...
            {view === "library" && !initializing && (
              <LibraryHub
                ...
                busy={busy}
                onOpen={async (id) => {
                  if (id !== book?.book_id) await loadCatalog(id);
                  setView("home");
                }}
                onRemove={async (id) => {
                  await api.removeBook(id);
                  const remaining = await api.books();
                  if (id === book?.book_id)
                    await loadCatalog(remaining[0]?.book_id, false);
                  else setBooks(remaining);
                }}
                onRestore={async (id) => {
                  await api.restoreBook(id);
                  if (!book) await loadCatalog(id, false);
                  else setBooks(await api.books());
                }}
            ...
            {view === "interview" && (
              <DiagnosticJourney
                ...
                onSelect={(id) =>
                  setSelected((current) =>
                    turn?.response_type === "multiple_choice"
                      ? current.includes(id)
            ...
            {view === "course" && course && (
              <CourseView
                course={course}
                bookId={book?.book_id ?? ""}
                onRemembered={returnsChanged}
                returnPanel={
                  book && (
                    <DoubtReturn
                      key={`${book.book_id}:${course.chapter_id}`}
                      bookId={book.book_id}
                      chapterId={course.chapter_id}
                      revision={returnRevision}
                      onAsk={revisitDoubt}
                    />
                  )
                ...
                confidence={confidence}
                activity={activity}
                needsRefresh={courseNeedsRefresh}
                busy={busy}
                onTab={(next) => {
                  setCourseTab(next);
                  setActivity(null);
                ...
                onRefresh={() => openCourse(course.chapter_id)}
              />
            )}
            {view === "qa" && !initializing && (
              <TutorChat
                question={qaQuestion}
                ...
                onBookChange={setQaBookId}
                currentBookId={book?.book_id}
                suggestions={suggestedQuestions(qaStructure)}
                ...
                followUp={
                  ...
                  (qaResult || error) && (
                    <RememberQuestion
                      ...
                      draft={{
                        book_id: qaBook.book_id,
                        question: askedQuestion,
                        chapter_id: qaStructure?.chapters.find((ch) =>
                          qaResult?.evidence_pages.some(
                            (p) => p >= ch.start_page && p <= ch.end_page,
                          ),
                        )?.chapter_id,
                        chapter_title: qaStructure?.chapters.find((ch) =>
                          qaResult?.evidence_pages.some(
                            (p) => p >= ch.start_page && p <= ch.end_page,
                          ),
                        )?.title,
                        excerpt: qaResult?.claims
                          .flatMap((c) => c.citations.map((s) => s.quote))
                          .join("\n")
                          .slice(0, 3000),
                        pages: qaResult?.evidence_pages.slice(0, 30),
                      }}
                      onSaved={returnsChanged}
                    />
                  )
                }
              />
            ...
            {view === "profile" && (
              <ProfileDashboard
                key={session?.session_id ?? book?.book_id}
                userProfile={userProfile}
                onAccount={() => setView("account")}
                profile={profile}
                sessionId={session?.session_id}
                bookTitle={book?.title ?? "未选择教材"}
                completed={completed}
                reminder={reminder}
                fontScale={fontScale}
                onReminder={(value) => {
                  setReminder(value);
                  safeSet("zhiwo.reminder", value);
                }}
                onFont={(value) => {
                  setFontScale(value);
                  safeSet("zhiwo.font-scale", String(value));
                }}
                onDiagnose={() =>
                  setView(completed ? "home" : session ? "interview" : "home")
                }
              />
            )}
            {view === "account" && (
            ...
            {error && view !== "qa" && (
              <div className="error-banner" role="alert">
                <span>{error}</span>
                <button
                  onClick={() => (!book ? void loadCatalog() : setError(""))}
                >
                  {!book ? "重新连接" : "知道了"}
                </button>
              </div>
          ...
          {notice && (
            <div className="notice-toast" role="status" key={notice}>
          ...
          {view !== "interview" && view !== "course" && view !== "account" && (
            <nav
              inert={busy || initializing}
              ...
              style={
                {
                  "--nav-index": [
                    ...
                    "qa",
                    "profile",
                  ].indexOf(view === "social" ? "community" : view),
            ...
            >
              ...
              />
              <NavButton
                active={view === "community" || view === "social"}
                icon="community"
                label="社区"
...
function CourseView({
  ...
  return (
    <div className="course-page" data-tab={tab}>
      <section className="course-hero">
        <div>
          <span className="depth-chip">
            {depthLabel[course.decision.depth]}
          </span>
          <h2>{course.chapter_title}</h2>
          <p>{course.opening}</p>
        </div>
        <div className="course-meta">
          <span>约 {course.estimated_minutes} 分钟</span>
          <span>{course.knowledge_points.length} 个知识点</span>
        </div>
      </section>
      {needsRefresh && (
      ...
      <StudioEntry
        anchor={{
          ...
          excerpt: `${course.chapter_title}\n${course.opening}\n${course.summary}`.slice(0, 3000),
        }}
      />
      <CourseTabs value={tab} busy={busy} onChange={onTab} />
      <div
        id="course-panel"
        role="tabpanel"
        aria-labelledby={`course-tab-${tab}`}
        className="course-panel tab-transition"
        key={tab}
      >
        {tab === "guide" && (
        ...
        {tab === "reading" && (
          <div className="content-stack">
            {course.original_reading.map((item) => (
              <ReadingCard
                key={`${item.title}-${item.content.slice(0, 10)}`}
                item={item}
                excerpt
                anchor={{
                  book_id: bookId,
                  chapter_id: course.chapter_id,
                  chapter_title: course.chapter_title,
                  pages: item.citations.map((c) => c.page_number).slice(0, 30),
                }}
                followUp={
                  <RememberQuestion
                    draft={{
                      ...
                      chapter_title: course.chapter_title,
                      question:
                        `请帮我理解「${item.title}」：${item.content.slice(0, 1000)}`.slice(
                          0,
                          2000,
                        ),
                      excerpt: item.content.slice(0, 3000),
                      pages: item.citations
                        .map((c) => c.page_number)
                        .slice(0, 30),
                    }}
        ...
        {tab === "points" && (
          <div className="content-stack">
            {course.knowledge_points.map((point) => (
              <section className="point-card" key={point.point_id}>
                <div>
        ...
        {tab === "practice" && practice && (
          <div className="practice-zone tab-transition" key={practice.item_id}>
            <div className="practice-head">
              <span>
                随堂练习 {practiceIndex + 1}/{course.practice_items.length}
              </span>
              <small>约 {practice.estimated_seconds} 秒</small>
            </div>
            ...
            <label className="confidence-control">
              ...
              <input
                ...
                value={confidence}
                onInput={(event) =>
                  onConfidence(Number(event.currentTarget.value))
                }
              />
            </label>
            <button
            ...
            <div className="practice-nav">
              <button
                ...
                onClick={() => onPractice(practiceIndex - 1)}
              >
                上一题
              </button>
              <button
                disabled={
                  busy || practiceIndex === course.practice_items.length - 1
                }
                onClick={() => onPractice(practiceIndex + 1)}
              >
                下一题
...
function formatPages(citations: Array<{ page_number: number }>) {
  return [...new Set(citations.map((citation) => citation.page_number))].join(
    "、",
  );
}
...
function ReadingCard({
  ...
  return (
    <section className="reading-card">
      <div>
        <strong>{excerpt ? "原文摘录" : item.title}</strong>
        <span>{item.purpose}</span>
      </div>
      {anchor ? (
        <StudyPassage anchor={anchor} text={item.content} />
      ) : (
        <p>{excerpt ? `…${item.content}…` : item.content}</p>
      )}
      <small>
        {excerpt && "省略号表示摘录边界 · "}教材第 {formatPages(item.citations)}{" "}
        页
      </small>
      {followUp}
    </section>
