...
export function LibraryShelf({books,activeBook,busy,onOpen}:{books:BookCatalogItem[];activeBook:BookCatalogItem|null;busy:boolean;onOpen:(id:string)=>void}) {
  ...
  return <div className="shelf-page">
    <section className="shelf-intro"><span className="kicker">我的书架</span><div><h2>你的书，安静地在这里。</h2><span>{books.length} 本</span></div><p>选择一本继续，解析内容和学习进度都已保存。</p></section>
    ...
    <div className="shelf-filters" aria-label="书架筛选"><button aria-pressed={!currentOnly} onClick={()=>setCurrentOnly(false)}>全部 <span>{books.length}</span></button><button aria-pressed={currentOnly} onClick={()=>setCurrentOnly(true)}>正在读</button><small><Icon name="check" size={13}/> 已同步</small></div>
    <div className="visual-shelf">{shown.map((book,index)=><article className="shelf-volume" key={book.book_id} style={{"--book-order":Math.min(index,5)} as React.CSSProperties}>
      <button className="shelf-cover" disabled={busy} aria-label={`预览《${book.title}》`} onClick={()=>setPreview(book)}><Cover book={book} eager={index<4}/></button>
