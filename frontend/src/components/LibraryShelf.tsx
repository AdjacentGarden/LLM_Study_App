...
export function LibraryShelf({books,activeBook,busy,onOpen}:{books:BookCatalogItem[];activeBook:BookCatalogItem|null;busy:boolean;onOpen:(id:string)=>void}) {
  ...
  return <div className="shelf-page">
    ...
    <div className="visual-shelf">{shown.map((book,index)=><article className="shelf-volume" key={book.book_id} style={{"--book-order":Math.min(index,5)} as React.CSSProperties}>
      <button className="shelf-cover" disabled={busy} aria-label={`预览《${book.title}》`} onClick={()=>setPreview(book)}><Cover book={book} eager={index<4}/><span className="cover-peek"><Icon name="spark" size={13}/>轻触预览</span></button>
      <button className="shelf-caption" disabled={busy} title={book.title} onClick={()=>onOpen(book.book_id)}><h3>{book.title}</h3><p>{book.page_count} 页<span>·</span>{book.chapter_count} 章</p>{book.book_id===activeBook?.book_id&&<span className="reading-ribbon">正在阅读</span>}<span className="shelf-enter">进入学习 <Icon name="arrow" size={15}/></span></button>
    </article>)}</div>
