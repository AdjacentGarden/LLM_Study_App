...
export function CommunityPage({books,onLibraryChanged,onOpenLibrary,onSocial}:{onSocial:()=>void;books:BookCatalogItem[];onLibraryChanged:()=>Promise<void>;onOpenLibrary:()=>void}) {
  ...
  return <div className="community-page"><section className="community-hero"><div><span className="kicker">社区精选</span><h2>发现别人认真读过的内容。</h2><p>带走一本书，也把自己的理解留给同路人。</p></div><button onClick={()=>setSharing(true)} aria-label="分享书籍或笔记"><Icon name="send" size={17}/><span>分享</span></button></section>
    ...
    <div className="community-section-title"><div><small>{kind==="book"?"大家分享的书":"最近公开的笔记"}</small><h3>{kind==="book"?"值得一起读":"从别人的思路里获得灵感"}</h3></div><span>{items.length} 份</span></div>
    ...
    <p className="community-fineprint">好内容值得分享。请只分享有权传播的书籍与笔记，彼此尊重创作。</p>
