...
export function UserProfilePage({onSaved,onBack,onBusy}:{onSaved:(value:UserProfile)=>void;onBack:()=>void;onBusy:(value:boolean)=>void}) {
  ...
  useEffect(()=>{let cancelled=false;void socialApi.me().then(value=>{if(!cancelled)setPublicId(value.user_id);}).catch(()=>{});return()=>{cancelled=true;};},[]);
